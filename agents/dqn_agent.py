...
class Agent:
    ...
    @tf.function
    def train_step(self, state_batch, action_batch, reward_batch, next_state_batch, done_batch) -> float:
        """
        ...
        targets = reward_batch + tf.subtract(1.0, tf.cast(done_batch, tf.float32)) * config.GAMMA * max_next_q
