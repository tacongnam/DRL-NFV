...
class RandomFit(RoutingMixin, Strategy):
    ...
    def get_placement(self, sfc: SFC, current_time: float) -> Optional[Dict]:
        ...
        for vnf in sfc.request.vnfs:
            ...
            valid_dcs = [
                str(dc) for dc in candidate_dcs
                if self.env.network.nodes.get(str(dc)) is not None
                and self.env._check_can_deploy_vnf(
                    self.env.network.nodes[str(dc)], vnf, t_start, t_end)
            ]
            if not valid_dcs:
                return None
            ...
            chosen_dc = path = None
            for dc in valid_dcs:
                p = self.get_routing(prev_dc, dc, t_start, t_end, sfc.request.bw)
                if p is not None:
                    chosen_dc, path = dc, p
                    break
