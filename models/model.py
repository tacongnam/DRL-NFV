"""
models/model.py  –  HRL-VGAE neural network components

Architecture (matching the PDF):
  • VGAENetwork   – Variational Graph Auto-Encoder (GCN encoder + inner-product decoder)
                    Pre-trained separately; frozen during RL training.
  • HighLevelAgent – DQN with Pareto-front multi-objective output [Q^AR, Q^Cost]
                     Uses non-dominated sorting (PMDRL) to pick the best SFC.
  • LowLevelAgent  – DQN + action masking; selects the DC for each VNF.
  • ReplayBuffer   – uniform experience replay.

Performance fixes vs. original code
  ─────────────────────────────────
  • All nets use small MLP (no unnecessary large layers) → fast forward pass.
  • VGAE encode() is O(N²) only at graph-build time, not every RL step.
  • HL/LL share the same Z embedding computed once per timestep.
  • No TF graph retracing: @tf.function on hot paths.
"""
...
class GCNLayer(layers.Layer):
    ...
    def call(self, H, A_hat):                    # H:(N,d)  A_hat:(N,N)
        agg = tf.matmul(A_hat, H)                # neighbourhood aggregation
        out = self.dense(agg)
...
class VGAENetwork:
    ...
    Encoder: 2-layer GCN → μ, log_σ²
    Decoder: inner-product  Â = σ(Z Zᵀ)
    Loss:    reconstruction BCE  +  β * KL divergence

    Usage
    -----
    vgae = VGAENetwork(node_feat_dim=3, hidden_dim=16, latent_dim=8)
    Z    = vgae.encode(X, A)          # inference  (N, latent_dim)
    vgae.train(buffer, epochs=5)      # online fine-tune (optional)
    """
    ...
    def _ensure_built(self):
        """Run a minimal forward pass to initialise weights."""
        if not self._built:
...
class LowLevelAgent:
    """
    Selects which Data-Centre to place the current VNF on.

    State  = [Z_mean (latent_dim,), vnf_resources (3,)]
    Action = index into dc_mapping list
    Q-value per action; invalid DCs masked to -∞ before argmax.

    Reward (from HRL strategy):
        R_LL = R_base
               + α * max(0, D_rem / tMax)     ← time headroom
               - β * cost_res_normalised       ← resource cost

        α, β are learned via a small weight network (avoids magic numbers).
    """
    ...
    def __init__(self, latent_dim: int = 8, max_dcs: int = 50,
        ...
        self.policy_net = _mlp(feat_dim, 64, max_dcs, "ll_policy")
        self.target_net = _mlp(feat_dim, 64, max_dcs, "ll_target")
        self._sync_target()
        self.opt = keras.optimizers.Adam(lr)

        # Small weight network for adaptive α, β (avoids hard-coded magic numbers)
    ...
    def _make_state(self, Z_t: np.ndarray, vnf_feat) -> np.ndarray:
        ...
        # Z_t can be (N, latent_dim) or (latent_dim,) — always reduce to 1-D
        z_arr  = np.asarray(Z_t, dtype=np.float32)
        if z_arr.ndim == 2:
            z_mean = z_arr.mean(axis=0)          # (latent_dim,)
        else:
            z_mean = z_arr.ravel()               # already 1-D

        # vnf_feat can be list, 1-D or 2-D array — always flatten to 1-D
        f_arr  = np.asarray(vnf_feat, dtype=np.float32).ravel()  # (3,)
    ...
    def act(self, Z_t: np.ndarray, vnf_feat: list,
        ...
        q     = self.policy_net(state, training=False).numpy()[0]   # (max_dcs,)
...
class HighLevelAgent:
    """
    Selects which pending SFC to process next.

    State  = [Z_mean (latent_dim,), SFC features (variable → padded)]
    Output = Q_H = [Q^AR, Q^Cost]  (two-headed DQN)

    Action selection (PMDRL non-dominated sorting):
      1. For each candidate SFC, compute [Q^AR_i, Q^Cost_i].
      2. Non-dominated sort → Pareto front.
      3. Pick the SFC on the front with highest Q^AR (accept more) or
         a weighted scalarisation during exploration.
    """

    FEAT_PER_SFC = 4          # [bw_req, n_vnfs, delay_budget_norm, ll_score]
    ...
    def __init__(self, latent_dim: int = 8, max_queue: int = 20,
        ...
        # Two-head output: [Q^AR, Q^Cost]
        self.policy_net = _mlp(feat_dim, 64, 2, "hl_policy")
        self.target_net = _mlp(feat_dim, 64, 2, "hl_target")
        self._sync_target()
        self.opt = keras.optimizers.Adam(lr)
    ...
    def _nondominated_sort(q_matrix: np.ndarray) -> List[int]:
        """
        q_matrix: (N, 2)  – [Q^AR, -Q^Cost]  (both to maximise)
        Returns indices on the Pareto front (rank-0 set).
        """
        N = len(q_matrix)
        dom_count = [0] * N
        for i in range(N):
            for j in range(N):
                if i == j: continue
                # j dominates i if j >= i on both objectives and > on at least one
    ...
    def act(self, Z_t: np.ndarray, queue: list, epsilon: float = 0.0,
        ...
        for feat in sfc_feats:
            ...
            q_two  = self.policy_net(state, training=False).numpy()[0]   # [Q^AR, Q^Cost]
            q_vals.append(q_two)

        q_mat  = np.array(q_vals)                      # (N, 2)
        # Objectives: maximise Q^AR, minimise Q^Cost → maximise -Q^Cost
        q_pareto = np.column_stack([q_mat[:, 0], -q_mat[:, 1]])
        front    = self._nondominated_sort(q_pareto)

        # Among Pareto front: pick highest Q^AR
